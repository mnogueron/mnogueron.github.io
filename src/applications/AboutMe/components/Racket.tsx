...
type RacketProps = {
  ...
  onDotClick: (event: React.UIEvent<SVGCircleElement>, id: string) => void;
} & Partial<HTMLChakraProps<'svg'>>;

type DotProps = {
  variant: 'light' | 'medium' | 'full';
  onClick: (event: React.UIEvent<SVGCircleElement>, id: string) => void;
...
const Dot = ({variant, onClick, ...props}: DotProps) => {
  const handleClick = (e: React.UIEvent<SVGCircleElement>) => {
    if (onClick && props.id) {
      onClick(e, props.id);
    }
  };
